    ...
    stages {
        ...
        */
        ...
        stage('Run Docker DataBase Image') {
            steps {           
            	withCredentials([string(credentialsId: 'con-mysql', variable: 'c-mysql-psw')]) {
				    bat 'docker run -it --name mysql-container --network my-network -e MYSQL_ROOT_PASSWORD=${c-mysql-psw} -e MYSQL_DATABASE=jspDiary -p 3306:3306 db-java-jsp-diary'
				}             
            }
        }
        ...
        stage('Docker Run Project Image') {
            steps {        	
                bat 'docker run -it --name jspDiary-container --network my-network -p 8080:8080 java-jsp-diary'
            }
        }
